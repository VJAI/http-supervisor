 ...
 */
export default class HttpSupervisorWidget extends HTMLElement {
  ...
  constructor() {
    ...
    [this._startButton, this._stopButton, this._clearButton, this._printButton, this._exportButton, this._callsCountLabel] = Array.from(shadowRoot.querySelector('#container').children);
    ...
    this._startButton.addEventListener('click', () => console.log('hi there'));
  }
  ...
  render() {
    ...
    this.el = document.createElement('http-supervisor-widget');
  ...
  destroy() {
    this.el.remove();
    this.el = this._startButton = this._stopButton = this._clearButton = this._printButton = this._exportButton = this._callsCountLabel = null;
  }
}

window.customElements.define('http-supervisor-widget', HttpSupervisorWidget);
