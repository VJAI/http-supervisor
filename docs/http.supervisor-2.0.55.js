    ...
    value: function duplicates(id, collection) {
      ...
      if (collection) {
        requestIds = collection.items.filter(function (item) {
          return item.url === url && item.method === method && JSON.stringify(item.payload) === JSON.stringify(payload);
        });
      } else {
        var reqSummary = this._callsSummary.find(function (r) {
          return r.url === url && r.method === method && JSON.stringify(r.payload) === JSON.stringify(payload);
        });

        requestIds = reqSummary ? toConsumableArray_default()(reqSummary.requests) : [];
      }
  ...
  createClass_default()(HtmlSupervisorWidgetElement, [{
    ...
    value: function _handleStopWatchChange(ctrl) {
      ...
      var setWidth = function setWidth() {
        var _this4$_container$get = _this4._container.getBoundingClientRect(),
            width = _this4$_container$get.width;

        _this4._container.style.right = "calc(50% - ".concat(width / 2, "px)");
      };
